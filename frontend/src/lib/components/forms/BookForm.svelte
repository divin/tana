<script lang="ts">
    ...
    let pages = $derived(book?.pages?.toString() || "");
    let rating = $derived(book?.rating?.toString() || "");
    let startedDate = $derived(book?.started_date || "");
    let completedDate = $derived(book?.completed_date || "");
    let notes = $derived(book?.notes || "");
    let coverPath = $derived(book?.cover_path || "");
    let error = $state<string | null>(null);
    let saving = $state(false);

    const isEditMode = $derived(!!book);
