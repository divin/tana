<script lang="ts">
    ...
    let releaseYear = $derived(movie?.release_year?.toString() || "");
    let director = $derived(movie?.director || "");
    let rating = $derived(movie?.rating?.toString() || "");
    let watchedDate = $derived(movie?.watched_date || "");
    let notes = $derived(movie?.notes || "");
    let posterPath = $derived(movie?.poster_path || "");
    let error = $state<string | null>(null);
    let saving = $state(false);

    const isEditMode = $derived(!!movie);
